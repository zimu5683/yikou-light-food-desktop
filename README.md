...
程序**不会**在 Wayland 会话里替你设置 `GTK_IM_MODULE`：Wayland 上 GTK3 默认走合成器的
text-input 协议（`im-wayland.so`），中文输入本来就是正常的；强行设成 `ibus`/`fcitx`
会换成传统的 X11 输入法模块，候选框不出现、甚至完全打不出中文。

- **Wayland（GNOME/KDE 默认）**：什么都不用做。若你自己在环境里配了 `GTK_IM_MODULE`，
  程序会尊重它、不覆盖。
- **X11 会话**：GTK3 无法从 `XMODIFIERS` 推断输入法模块，程序会按**实际在跑的**
  输入法守护进程（fcitx5 / fcitx / ibus）补上 `GTK_IM_MODULE`，探测不到才退回
  环境变量提示（这是 X11 下"输入框只能打英文"的常见原因）。
- 仍然打不出中文时，先用 `echo $XDG_SESSION_TYPE` 确认会话类型，再在终端里运行
  `env GTK_IM_MODULE=ibus ./yikou-light-food` 对比一次即可定位。
...
- 排序辅助列的位置按**真实内容宽度**算，不直接信接口报的 used range ——
  后者被整列操作污染后会停在网格最右列（东湖中餐实测 16384），
  旧版据此算出的辅助列超过上限，排序会被**静默跳过**；
- 排序前后各探一次「辅助列右侧 40 列」：那里若有内容，说明排序区覆盖不到它，
  程序**放弃排序并回滚新行**（宁可不排，也不把行与列排错位），日志会点名具体单元格；
- 任何原因导致「本次没有按地址重排」时，日志都会以 ⚠ 单独打一行说明，
  不再只在预览里显示一行小字；

...
- 辅助列放在**该表所有内容列的右侧**，排序范围 `A3:<辅助列><末行>`
  —— 必须覆盖全部内容列，否则右边的列不跟着行走会错位；
  排序后整列删除（失败则退化为写空串清空）；
  - ⚠️ **2026-09-16 修复：宽度不能无条件取 `sheetsInfo.colTo`**。它是 WPS 记的
    used range 右下角，被"整列/到最右列"的操作碰过之后会停在网格最右列。
    真机实测**东湖中餐返回 `colTo=16383`**（其余 5 张表分别是 127/124/100/99/96，
    都正常），于是辅助列算成 16385、超过 `MAX_SORT_COL=1000`，
    `sort_on` 被静默关掉 —— **这张表从来没排过序**，新客户一直留在第 4 行起。
    现在由 `effective_last_col()` 判定：报出来的宽度没超过实际读取范围（`MAX_SCAN_COL`）
    就照用（更保守，辅助列离内容更远，不会去推内容右侧的边框/底色）；
    超过则退回读取宽度，边界由 `probe_sort_area()` 兜底；
  - 排序前/排序后各探一次「辅助列右边那一带」（`SORT_PROBE_WIDTH=40` 列，只读一次）：
    发现内容说明排序区覆盖不到它，**拒绝排序并回滚插入**，宁可不排也不排错位；
    探测从 `辅助列 + 1` 起，别把辅助列自己写的排序键读成"右侧有内容"；
...
2026-09-16 起排序前/后各多 1 次右侧边界探测（只对"本次真的要排序"的表发），
即**每张排序表再多 2 次**，6 张表每天约多 47 次；
预览 + 上传合计约 150 次/天，仍在 150~200 的上限内。

### 13.5.1 2026-09-16 修复：东湖中餐排不了序（真机只读复验）

**现象**（用户反馈）：云同步后东湖中餐的新客户没有按地址归位。

**根因**：`sheetsInfo.colTo` 被整列污染成 16383 → 辅助列 16385 → 超过
`MAX_SORT_COL` → `sort_on=False`，只在预览里留一行不显眼的"表格宽度异常…本次跳过排序"。

**真机只读诊断（修复前 / 修复后）**：

| 子表 | 接口报 colTo+1 | 真实最后有内容的列 | 修复前辅助列 | 修复后辅助列 | 修复后排序区 |
|---|---|---|---|---|---|
| 东湖中餐 | **16384** | 22 (V) | 16385 ❌ 跳过 | 202 | `A3:GT105` ✅ |
| 衣锦中餐 | 128 | 128 | 129 | 129 | — |
| 医学院中餐 | 126 | 126 | 127 | 127 | `A3:DW32` ✅ |
| 东湖晚餐 | 101 | 101 | 102 | 102 | `A3:CX37` ✅ |
| 衣锦晚餐 | 100 | 100 | 101 | 101 | `A3:CW12` ✅ |
| 医学院晚餐 | 97 | 97 | 98 | 98 | `A3:CT13` ✅ |

（衣锦中餐当日本地无单，未触发排序分支。）

**新增护栏**：辅助列右侧 40 列内有内容 → 拒绝排序 + 回滚插入 + 报告具体单元格。
另外"本次未排序"现在会以 `WARN` 级别单独打一行日志，不再只在预览里出现。
